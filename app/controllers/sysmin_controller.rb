class SysminController < ApplicationController
...
@userList  = TUser.find_by_sql( 
  ...
  def dept
    ...
    _sql_ = 'select b.dept_id,b.dept_name, b.dept_pid ,c.dept_name as dept_pName,b.updated_at from t_depts b left JOIN t_depts c on b.dept_pid = c.dept_id
              where  b.status = 0'
    @deptList = TDept.find_by_sql(_sql_);
  end
  def role
  ...
  def userSave
      ...
      if @_user.save
          redirect_to(:controller => "sysmin",:action => "user")
      end 
  end
  ...
  def roleSave
      ...
      if @_role.save
          redirect_to(:controller => "sysmin",:action => "role")
      end 
  end 
  ...
  def deptSave
      ...
      if @_dept.save
          redirect_to(:controller => "sysmin",:action => "dept")
      end 
  end
